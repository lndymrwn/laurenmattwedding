...
class Map extends React.Component {
  ...
  loadMap = () => {
    loadScript(
      "https://maps.googleapis.com/maps/api/js?key=AIzaSyCiD21v8wwCf9-QRTr8mM69JblmLXgxBwc&callback=initMap"
    );
  ...
  initMap = () => {
    const map = new window.google.maps.Map(document.getElementById("map"), {
      center: { lat: 20.2114185, lng: -87.4653502},
      zoom: 7,
      mapTypeId: 'satellite',
    });
    ...
    this.state.mapLocations.forEach(location => {
      ...
      <h2>${'some location name'}</h2>

      <p><img src="${'https://via.placeholder.com/150'}" />${'some image descriptive text'}
      <br /><em>${'some marker text'}</em></p>
      `;

      const marker = new window.google.maps.Marker({
        position: { lat: 20.2114185, lng: -87.4653502},
        map: map,
      });
      // var kmlLayer = new window.google.maps.KmlLayer("https://raw.githubusercontent.com/agalin920/MyKML/master/La.kml", {
